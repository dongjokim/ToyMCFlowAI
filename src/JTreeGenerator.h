...
class JTreeGenerator {
	public:
		...
		void GenerateTree();
		void AddEvent(int i, int cent, int Ntrk, double Psi_n[2], double v2, double v3);
        void AddTrack(int i, double px, double py, double pz, double E,double phi, double eta, double pt, double mass);
		void FillTree();
		void WriteTree();
		void AddEvent(Int_t random_seed, int iEvent, double cent, int ic, int Ntrk, double Psi_n[2], double v2, double v3);
